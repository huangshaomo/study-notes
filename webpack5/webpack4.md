...
第一步，首先读取入口模块文件，检查该模块文件id（文件路径名）是否已记录到依赖中（dependencies），如果已记录说明该模块已被加载过，结束。如果没有记录，则继续。

第二步，读取文件内容进行AST抽象语法树进行语法分析，在AST中找到导入语法声明，例如**CommonJs**是找到program下ExpressStatement下的callee和arguments对应的name，value属性值，合并起来就是require('xxx')
...
1. 声明一个对象，存放加载过模块
2. 其他无关声明，忽略....
3. 使用__webpack_require__调用入口文件(./src/index.js)

**\__webpack_require__**（）：

1. 检查模块是否被缓存过，如果缓存过，直接返回缓存的模块结果，结束
2. 格式化模块然后放入缓存中
3. 进入该模块的函数执行内部，并把this执行该模块的exports（停，先转入下面的函数内部）
4. 表示该模块已被加载
5. 返回该模块的exports
...
2. 如果存在，则给export对象内部定义一个Symbol的数据类型，值为字符串Module，格式如下
...
2. 调用**\__webpack_require__.r**()函数，并把index的exports对象穿过去(当然现在的exports对象是空的)

3. **\__webpack_require__.r**()函数后的index的exports是这样

   ![360chrome_Y0ad3XSw9U](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/360chrome_jh4dGEpppH.png)

4. 调用webpack内部封装的\__webpack_require__执行（./src/a.js）模块
...
2. 调用**\__webpack_require__.r**()函数，并把a的exports对象穿过去(当然现在的exports对象是空的)
   ...
   **\__webpack_require__.r**()函数后的a的exports是这样

   ![360chrome_Y0ad3XSw9U](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/360chrome_jh4dGEpppH.png)

3. 调用**\__webpack_require__.d**()函数，并把a的exports对象，字符串a，一个返回字符串a的函数，当作参数传递过去。

   **\__webpack_require__.d**()函数后的a的exports是这样
