...
![单线程与多线程的进程对比图](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200822153129121.png)
...
2. **线程之间共享进程中的数据**
   ...
   ![image-20200822153551207](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200822153551207.png)
...
![image-20200822154046799](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200822154046799.png)
...
![早期多进程架构](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200822160932997.png)
...
![目前多进程架构](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200822162557315.png)
...
![未来面向服务的架构（SOA）](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200822155914608.png)
...
![简化的IP网络三层传输模型](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200823113337270.png)
...
![简化的UDP网络四层传输模型](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200823114734153.png)
...
![简化的TCP网络四层传输模型](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200823211412161.png)
...
![image-20200823122439271](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200823213303163.png)
...
不过，先不急，在了解网络请求之前，我们需要先看看HTTP和TCP的关系。因为浏览器使用HTTP协议作为应用层协议，用来**封装请求的文本信息**，并使用TCP/IP作传输层协议将它发到网络上，所以在HTTP工作开始之前，浏览器需要通过TCP与服务器建立连接。也就是说**HTTP的内容是通过TCP的传输数据阶段来实现的**，你可以结合下图更好地理解这两种的关系。![TCP和HTTP的关系示意图](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200823214513129.png)
...
![HTTP请求数据格式](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200823122244462.png)
...
![服务器响应的数据格式](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200823214232818.png)
...
![image-20200823215442020](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200823215442020.png)
...
![HTTP请求流程示意图](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200824125311734.png)
...
![从输入URL到页面展示完整流程](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200823122439271.png)
...
![开始加载URL浏览器状态](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200824134521670.png)
...
![image-20200824134521670](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/chrome_HGHkt7r70q.png)
...
![image-20200824135019109](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200824135520385.png)
...
![含有HTML格式的Content-type](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/chrome_VUftS0HA1p.png)

从图中可以看到，响应头中的 Content-type 字段的值是 text/html，这就是告诉浏览器，服务器返回的数据是 HTML 格式。

然后我们再以请求一个安装包为例，返回的信息如下：
![含有Stream格式的Content-type](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/chrome_QOQA7SPCYE.png)
...
2. 如果从A页面打开B页面，且A和B都属于同一站点的话，那么B页面就会复用A页面的渲染进程，如果是其他情况，浏览器进程则会为B创建一个新的渲染进程
   ...
   ![chrome_VUftS0HA1p](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200824135637987.png)

   而如果直接输入tieba.baidu.com而不是从baidu.com页面点击进入的话，浏览器会为tieba的百度重新创建一个新的渲染进程

   ![chrome_QOQA7SPCYE](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200824135019109.png)
...
![导航完成状态](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828135002260.png)
...
![image-20200828131937322](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200824142451807.png)
...
![image-20200828135002260](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828141218461.png)
...
![image-20200828141218461](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828131937322.png)

![image-20200828142013038](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828141537666.png)

##### 阶段一：通过分词器将字节流转换为Token。

![生成token示意图](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828142013038.png)
...
![image-20200828143227354](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828143227354.png)

然后按照同样的流程解析出来 StartTag body 和 StartTag div，其 Token 栈和 DOM 的状态如下图所示

![image-20200828144012242](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828144012242.png)

接下来解析出来的是第一个 div 的文本 Token，渲染引擎会为该 Token 创建一个文本节点，并将该 添加到 DOM 中，它的父节点就是当前 Token 栈顶元素对应的节点，如下图所示：

![image-20200828144107484](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828144107484.png)

再接下来，分词器解析出来第一个 EndTag div，这时候 HTML 解析器会去判断当前栈顶的元素是否是 StartTag div，如果是则从栈顶弹出 StartTag div，如下图所示：

![image-20200828144233802](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828144233802.png)

按照同样的规则，一路解析，最终结果如下图所示：

![image-20200828144307357](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828145805224.png)
...
![image-20200828145805224](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828144307357.png)
...
![image-20200828160441217](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828160441217.png)
...
![image-20200828160613887](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828160613887.png)
...
![image-20200828160647907](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828160647907.png)
...
![image-20200828180706119](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828180706119.png)
...
![image-20200828194811169](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828194811169.png)
...
![image-20200828213822580](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828213822580.png)
...
![image-20200828214627314](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828214627314.png)

比如一个标签很小，50*50像素，你在里面放了非常多的文字，那么超出的文字部分就需要被剪裁。当然如果出现了滚动条，那么滚动条也会被单独提升为一个图层，如下图

![image-20200828214917679](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828214917679.png)
...
![image-20200828215450698](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828215450698.png)

从图中可以看出，绘制列表中的指令其实非常简单，就是让其执行一个简单的绘制操作，比如绘制粉色矩形或者黑色的线等。而绘制一个元素通常需要好几条绘制指令，因为每个元素的背景、前景、边框都需要单独的指令去绘制。所以在图层绘制阶段，输出的内容就是这些待绘制列表。

你也可以打开“开发者工具”的“Layers”标签，选择“document”层，来实际体验下绘制列表，如下图所示：

![chrome_9EXSoE9jJQ](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/chrome_9EXSoE9jJQ.png)
...
![image-20200828220604059](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828220604059.png)
...
![image-20200828221419897](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828221419897.png)
...
![image-20200828221541911](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828221541911.png)

通常，栅格化过程都会使用 GPU 来加速生成，使用 GPU 生成位图的过程叫快速栅格化，或者 GPU 栅格化，生成的位图被保存在 GPU 内存中。

相信你还记得，GPU 操作是运行在 GPU 进程中，如果栅格化操作使用了 GPU，那么最终生成位图的操作是在 GPU 中完成的，这就涉及到了跨进程操作。具体形式你可以参考下图：

![image-20200828221636212](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828221636212.png)
...
![image-20200828221840594](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828221840594.png)

![image-20200828224009140](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828224009140.png)
...
![chrome_OhTiUbymKO](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/chrome_OhTiUbymKO.png)
...
![image-20200828224755802](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828224755802.png)
...
![image-20200828225103410](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200828225103410.png)
...
![image-20200921092733327](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200921093555903.png)
...
![image-20200921093555903](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200921092733327.png)
...
![image-20200921094822595](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200921100542474.png)
...
![image-20200921095557903](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200921095557903.png)
...
![image-20200921100542474](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200921113309191.png)
...
![image-20200921101341820](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200921101341820.png)
...
![image-20200921101654181](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200921094822595.png)
...
![image-20200921113309191](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200921115747318.png)
...
![image-20200921114221026](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200921101654181.png)
...
![image-20200921114634429](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200921114221026.png)
...
![image-20200921115747318](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200921134009320.png)
...
![image-20200921133607357](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200921133607357.png)
...
![image-20200921134009320](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/chrome_6QwoOkCOix.png)
...
![chrome_6QwoOkCOix](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/chrome_mgx5hXXvOv.png)
...
![chrome_mgx5hXXvOv](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/chrome_1aPBVg4Mfc.png)
...
<img src="https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/image-20200921114634429.png" alt="chrome_1aPBVg4Mfc" style="zoom:80%;" />
...
![chrome_pO5xa1F9NF](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/chrome_pO5xa1F9NF.png)
...
![chrome_f3edZ2KYzg](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/chrome_f3edZ2KYzg.png)

当作用域块执行结束之后，其内部定义的变量就会从词法环境的栈顶弹出，最终执行上下文如下图所示：

![chrome_GQrK88i5jt](https://hsm-typora-img.oss-cn-beijing.aliyuncs.com/img/chrome_GQrK88i5jt.png)
